...
Shader "Effect/Distortion/CullBack" {
Properties {
...
Category {
	...
	SubShader {
		...
		Pass {
			...
			sampler2D _MySrcTexture;
			float4 _MySrcTexture_TexelSize;
			fixed4 _TintColor;
			...
			v2f vert (appdata_t v)
			{
				...
				if(_MySrcTexture_TexelSize.y < 0)
					scale = -1.0;
				#else
				#endif
			...
			half4 frag( v2f i ) : COLOR
			{
				...
				float2 offset = bump * _BumpAmt * _MySrcTexture_TexelSize.xy;
				i.uvgrab.xy = offset * i.uvgrab.z + i.uvgrab.xy;
				half4 col = tex2Dproj( _MySrcTexture, UNITY_PROJ_COORD(i.uvgrab));
